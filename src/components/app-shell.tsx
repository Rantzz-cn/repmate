...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <AppProvider><div className={cn("app-shell grid-bg", focusedPage && "app-shell--focus")}><aside className="app-sidebar"><div className="app-sidebar__logo"><Image src="/assets/images/whitelogo.png" width={210} height={210} alt="RepMate" priority/></div>{!focusedPage && <nav className="app-sidebar__nav" aria-label="Primary navigation">{links}</nav>}</aside><div className="app-content"><header className="app-mobile-brand"><div className="app-brand-crop"><Image src="/assets/images/whitelogo.png" width={210} height={210} alt="RepMate" priority/></div></header><main className="app-main">{children}</main></div>{!focusedPage && <nav className="app-bottom-nav safe-bottom" aria-label="Primary navigation">{links}</nav>}</div></AppProvider>;
}
