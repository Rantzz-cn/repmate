...
export function ShareRecap({ workout, profile, onClose }: { workout: Workout | null; profile: Profile; onClose(): void }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="overflow-y-auto p-5"><DialogTitle>Your workout recap</DialogTitle><DialogDescription className="mt-1">Formatted for Instagram and Facebook stories.</DialogDescription>{image ? <img src={image} alt="RepMate workout recap" className="mx-auto mt-4 max-h-[65dvh] rounded-2xl border border-white/10" /> : <div className="numeric grid h-72 place-items-center text-zinc-600">Building recap</div>}<div className="mt-4 grid grid-cols-2 gap-2"><Button variant="secondary" onClick={save} disabled={!image}><Download className="size-4" />Save image</Button><Button onClick={share} disabled={!image}><Share2 className="size-4" />Share</Button></div></DialogContent></Dialog>;
}
